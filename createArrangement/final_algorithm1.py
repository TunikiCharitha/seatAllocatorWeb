...
import math
import time
#from createArrangement.demo2 import result2
#room_section_rollnos = [[] for i in range(len(utilised_rooms))]
def result(request):
    getobjects2()
    getobjects1()
    getobjects3()
    path = 'C:\\Users\\DELL\\Desktop\\seatAllocatorWeb\\file1.py'
    l1 = list()
    fp1 = open(path, 'r')
    for line in fp1:
    ...
    for line in fp2:
        for word in line.split():
            l2.append(word)
    print(l2)
    path = 'C:\\Users\\DELL\\Desktop\\seatAllocatorWeb\\file3.py'
    l3 = list()
    fp3 = open(path, 'r')
    for line in fp3:
        for word in line.split():
    ...
    if es >= 0:
        ...
        workbook.close()
        deletion()
    return render(request, 'createArrangement/index4.html',{'name':timestr1})
